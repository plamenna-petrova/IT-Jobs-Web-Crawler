...
def export_job_posts_scraping_results_to_csv(
    ...
    try:
        if it_jobs_posts_list:
            ...
            print('No suitable job posts found. So the csv file was not created!')
    ...
    try:
        if it_job_posts_list:
            ...
            for i, it_job_post in enumerate(it_job_posts_list):
                with open(f'{txt_files_storage_directory_name}/{i + 1}.txt', 'w') as txt_file_writer:
...
def export_job_post_scraping_results_to_pdf(
    ...
    try:
        if it_job_posts_list:
            ...
            print('No suitable job posts found. So the pdf file was not created!')
    except Exception as exception:
        logger.error(str(exception))
